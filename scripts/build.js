    ...
    !function(){
        ...
        function captureDetail(detail){
            try{
                ...
                if(eventId&&sentEventIds.has(eventId))return true;
        ...
        function queuePendingDetail(detail){
            var eventId=detail&&typeof detail.eventId==="string"?detail.eventId:"";
            if(eventId&&(sentEventIds.has(eventId)||pendingEventIds.has(eventId)))return;
            pendingDetails.push(detail);
            if(eventId)pendingEventIds.add(eventId);
            if(pendingDetails.length>100){
                var removed=pendingDetails.shift();
                if(removed&&removed.eventId)pendingEventIds.delete(removed.eventId);
            }
        }
        function flushPendingDetails(){
            if(!window.posthog||typeof window.posthog.capture!=="function")return;
            var queued=pendingDetails.slice();
            pendingDetails=[];
            pendingEventIds.clear();
            queued.forEach(captureDetail);
        }
        window.addEventListener("anderseed:analytics",function(event){
            try{
                ...
                if(captureDetail(detail))return;
                if(document.documentElement.getAttribute("data-analytics-consent")==="pending")queuePendingDetail(detail);
            }catch(_error){}
        });
        document.addEventListener("anderseed:analytics-consent-changed",function(event){
            try{
                ...
                if(choice==="accepted")flushPendingDetails();
        ...
        <div class="assessment-gate-form-sheet">
          ...
          <form class="assessment-lead-form" data-lead-form novalidate>
            ...
            <label class="assessment-field" for="assessmentEmail">Email address <span aria-hidden="true">*</span>
              <input id="assessmentEmail" name="email" type="email" autocomplete="email" inputmode="email" aria-describedby="assessmentContactStatus" required maxlength="254" placeholder="Enter your email address" />
            </label>
            ...
            <p class="assessment-storage-status" id="assessmentContactStatus" data-storage-status role="alert" hidden></p>
